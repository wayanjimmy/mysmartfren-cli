...
const details = () => {
  ...
  .then(function (body) {
    ...
    let bonusInfo = $('table').eq(11);
    let bonusKuota = $(bonusInfo).find('tr').eq(0).find('td').eq(2).text();
    let berlakuBonus = $(bonusInfo).find('tr').eq(1).find('td').eq(2).text();
    let kuotaMalam = $(bonusInfo).find('tr').eq(2).find('td').eq(2).text();
    let berlakuKuotaMalam = $(bonusInfo).find('tr').eq(3).find('td').eq(2).text();

    table.push(['Nomor', nomor]);
    table.push(['Paket', paket]);
    table.push(['Kuota', kuota]);
    table.push(['Berlaku', berlaku]);
    table.push(['Bunus Kuota', bonusKuota]);
    table.push(['Berlaku', berlakuBonus]);
    table.push(['Kuota Malam', kuotaMalam]);
    table.push(['Berlaku', berlakuKuotaMalam]);
